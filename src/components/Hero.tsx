          ...
          <div className="text-center">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <p className="mx-auto mt-6 max-w-2xl text-xl text-stone-100 sm:text-2xl">
                  {slides[currentSlide].subtitle}
                </p>
                
                <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
                  <Button size="lg" className="h-14 rounded-full bg-white text-stone-900 px-8 text-lg hover:bg-stone-100">
                    {slides[currentSlide].cta} <ArrowRight className="ml-2 h-5 w-5" />
                  </Button>
                  <a href="#post-requirement">
                    <Button size="lg" variant="outline" className="h-14 rounded-full border-white/30 bg-white/10 text-white backdrop-blur-md px-8 text-lg hover:bg-white/20">
                      Post Requirement
                    </Button>
                  </a>
                </div>
            ...
            <motion.div 
              ...
              className="mx-auto mt-16 max-w-4xl rounded-3xl bg-white/95 p-6 shadow-2xl backdrop-blur-sm"
            >
              <Tabs defaultValue="buy" className="w-full">
                <TabsList className="mb-6 bg-stone-100/50 p-1 rounded-full">
                  <TabsTrigger value="buy" className="rounded-full px-8 data-[state=active]:bg-stone-900 data-[state=active]:text-white">Buy</TabsTrigger>
                  <TabsTrigger value="rent" className="rounded-full px-8 data-[state=active]:bg-stone-900 data-[state=active]:text-white">Rent / Lease</TabsTrigger>
                  <TabsTrigger value="sell" className="rounded-full px-8 data-[state=active]:bg-stone-900 data-[state=active]:text-white">Sell</TabsTrigger>
                </TabsList>
                
                <div className="grid gap-4 md:grid-cols-3">
                  <div className="relative">
                    <MapPin className="absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-stone-400" />
                    <Input className="h-14 pl-10 rounded-xl border-stone-200 bg-white" placeholder="Location in Chandigarh/Mohali" />
                  </div>
                  <div className="relative">
                    <Home className="absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-stone-400" />
                    <Input className="h-14 pl-10 rounded-xl border-stone-200 bg-white" placeholder="Property Type (Kothi, Flat, Land)" />
                  </div>
                  <Button className="h-14 rounded-xl bg-stone-900 text-lg hover:bg-stone-800">
